...
function init(){
  ...
  // Navigation buttons
  const btnViewSaved = $("btnViewSaved");
  if(btnViewSaved){
    btnViewSaved.addEventListener("click", () => window.location.href = "saved-inspections.html");
  }
  const mBtnViewSaved = $("mBtnViewSaved");
  if(mBtnViewSaved){
    mBtnViewSaved.addEventListener("click", () => window.location.href = "saved-inspections.html");
  }
  ...
  if(btnArchive){
    btnArchive.addEventListener("click", () => {
      saveInspectionArchive("avsluttet");
      alert("Befaringen er arkivert.");
    });
  }
...
function positionHelpPopover(anchorEl, pop){
  ...
  // Først: last fra arkiv hvis vi redigerer en lagret befaring
  const loadedFromArchive = loadInspectionForEditing();

  // Deretter: last tidligere lagret data hvis det finnes
...
function buildAttendeesArray(){
  const list = [];
  const klp = state.attendees?.klp || [];
  const cust = state.attendees?.customer || [];
  const customerName = state.customer?.name || "Kunde";

  klp.forEach(a => {
    if(!a || (!a.name && !a.title)) return;
    list.push({ name: a.name || "", title: a.title || "", employer: "KLP" });
  });
  cust.forEach(a => {
    if(!a || (!a.name && !a.title)) return;
    list.push({ name: a.name || "", title: a.title || "", employer: customerName });
  });
  return list;
}

function normalizeAttendeesObject(attendees, customerName){
  if(attendees && !Array.isArray(attendees) && attendees.klp && attendees.customer){
    return attendees;
  }
  const res = { klp: [], customer: [] };
  const arr = Array.isArray(attendees) ? attendees : [];
  arr.forEach(a => {
    const employer = (a.employer || "").toLowerCase();
    if(employer.includes("klp")){
      res.klp.push({ name: a.name || "", title: a.title || "" });
    } else {
      res.customer.push({ name: a.name || "", title: a.title || "" });
    }
  });
  if(res.klp.length === 0) res.klp.push({ name:"", title:"" });
  if(res.customer.length === 0) res.customer.push({ name:"", title:"" });
  return res;
}

function saveInspectionArchive(status = "avsluttet"){
  const inspections = JSON.parse(localStorage.getItem("savedInspections") || "[]");
  const id = state.archiveId || Date.now();
  const now = new Date().toISOString();

  const reportAuthor = (state.attendees?.klp || []).map(x => x.name).filter(Boolean)[0] || "";
  const attendeesArray = buildAttendeesArray();
  ...
  const payload = {
    ...
    savedDate: now,
    status,
    createdDate: state.archiveCreatedDate || now
  };
  ...
  state.archiveId = id;
  state.archiveCreatedDate = payload.createdDate;
  return id;
}
...
function loadInspectionForEditing(){
  ...
  try{
    ...
    state.inspectionDate = data.inspectionDate || state.inspectionDate;
    state.customer = data.customer || state.customer;
    state.locations = data.locations || state.locations;
    state.activeLocationId = data.activeLocationId || (state.locations?.[0]?.id || state.activeLocationId);
    state.findings = data.findings || data.items || [];

    const customerName = data.customer?.name || state.customer?.name || "Kunde";
    state.attendees = normalizeAttendeesObject(data.attendees, customerName);

    state.archiveId = data.id || state.archiveId;
    state.archiveCreatedDate = data.createdDate || state.archiveCreatedDate || new Date().toISOString();
